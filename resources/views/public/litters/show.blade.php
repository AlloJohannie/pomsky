          ...
          <div class="bg-white rounded-2xl border border-neutral-200 p-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-2 gap-6">
              @foreach($puppies as $puppy)
                @php
                  $photos = $puppy->photos;               // already ordered desc by taken_at/created_at
                ...
                <div class="group overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-sm">
                  <div class="relative">
                    <img src="{{ $cover }}" class="w-full aspect-[4/3] object-cover" alt="{{ $puppy->name ?: 'Chiot' }}">
                  ...
                  <div class="p-5 space-y-3">
                    <div class="flex items-center justify-between">
                      ...
                      @if(optional($photos->first())->week !== null)
                        <div class="text-xs text-slate-500">
                          {{ $weekLabel(optional($photos->first())->week) }}
                        </div>
                      @endif
                    </div>
                    ...
                    {{-- Galerie (12 dernières) --}}
                    @if($photos->count())
                      <div class="mt-2">
                        <div class="flex flex-wrap gap-2">
                          @foreach($photos->take(12) as $ph)
                            <div class="relative w-20 h-20 rounded-lg overflow-hidden border border-neutral-200 bg-neutral-50">
                              <img src="{{ asset('storage/'.$ph->path) }}" class="w-full h-full object-cover" alt="">
                              @if(!is_null($ph->week))
