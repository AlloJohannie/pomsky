        ...
        <div class="container">
            <div class="lg:w-4/10 md:w-7/10 mx-auto" data-aos="fade-up" data-aos-delay="150" data-aos-duration="500" data-aos-easing="ease-in-out">
                <div class="lg:mb-12.5 md:mb-10 mb-7.5 text-center">
                    <h1 class="lg:text-6xl md:text-5.5xl text-4xl">Forgot password</h1>
                    <p class="mb-2.5">Enter your registered email to receive a reset link</p>
                </div>

                <div class="bg-body-bg md:p-10 p-5 rounded-2xl">
                    <form action="{{ route('password.email') }}" method="POST" class="mb-3.75">
                        @csrf

                        <!-- Email -->
                        <div class="mb-5">
                            <label for="email" class="mb-1.25 block font-normal">Email Address</label>
                            <input class="rounded-2xl py-2.5 px-5 border border-neutral-200 w-full h-14 @error('email') border-red-500 @enderror" name="email" id="email" type="email" placeholder="Enter your email" maxlength="255" required autofocus>
                            @if (session('status'))
                                <ul class="list-unstyled ps-0 mt-1">
                                    <li class="text-green-600 mb-1">{{ session('status') }}</li>
                                </ul>
                            @endif
                            @if ($errors->get('email'))
                                <ul class="list-unstyled ps-0 mt-1">
                ...
                <div class="flex justify-between md:gap-5 md:flex-row gap-2.5 flex-col mt-7.5 items-center">
                    <a href="{{ route('any', 'index') }}">
                        <img src="/images/logo/landinger.svg" alt="" class="h-9 w-38.75">
                    </a>
                    <div class="text-sm">
